...
tar zcf $RPM_SOURCES_DIR/kaltura-postinst-$KALTURA_POSTINST_VERSION-TM.tar.gz postinst/
cd -
echo "Packaged into $RPM_SOURCES_DIR/kaltura-postinst-$KALTURA_POSTINST_VERSION.tar.gz"
if [ -x "`which rpmbuild 2>/dev/null`" ];then
	rpmbuild -bb `dirname $0`/../spec_files/kaltura-postinst.spec --define "_rpmdir $RPM_CREATION_DIR/"
fi
