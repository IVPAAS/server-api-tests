...
RPMMACROS_TEMPLATE="/opt/sandbox/master_packer/rpmmacros.template"
TEMP_RPMMACROS_FILE="/opt/sandbox/master_packer/rpmmacros.altered"
...
RPM_CREATION_DIR="/home/$PACKING_USER/rpmbuild/RPMS"
FINAL_REMOTE_RPM_PATH="/var/www/html/onPremRepository/autobuilds/base_builds"
REMOTE_RPM_REPO_MACHINE="onpremrepo.dev.kaltura.com"
PACKAGES_SCRIPT_LIST="package_kaltura_core.sh"
CE_REPO_URL="http://installrepo.origin.kaltura.org/repo/releases/@VERSION@/RPMS/"
# CE_RPM_TEMP_DOWNLOAD="/tmp/ce_temp_download"
...
function clean_up_dirs () {
    ...
    for dir in $RPM_CREATION_DIR; do
...
function move_to_final_repository () {
    build_tag=$1
    RSYNC=`which rsync`
    RSYNC_COMMAND="$RSYNC -i -v -r $RPM_CREATION_DIR/noarch/* root@$REMOTE_RPM_REPO_MACHINE:$FINAL_REMOTE_RPM_PATH/$build_tag/"
    echo $RSYNC_COMMAND
    $RSYNC_COMMAND
}
